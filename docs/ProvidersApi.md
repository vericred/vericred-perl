...
# **get_providers_0**
> ProvidersGeocodeResponse get_providers_0(body => $body)
...
eval { 
    my $result = $api_instance->get_providers_0(body => $body);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling ProvidersApi->get_providers_0: $@\n";
}
```
