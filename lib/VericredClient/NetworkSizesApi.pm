...
# @param StateNetworkSizeRequest $body  (required)
{
    my $params = {
    'body' => {
        data_type => 'StateNetworkSizeRequest',
        description => '',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'search_network_sizes' } = { 
    	summary => 'Network Sizes',
        params => $params,
        returns => 'StateNetworkSizeResponse',
        };
}
# @return StateNetworkSizeResponse
#
sub search_network_sizes {
    my ($self, %args) = @_;

    # verify the required parameter 'body' is set
    unless (exists $args{'body'}) {
      croak("Missing the required parameter 'body' when calling search_network_sizes");
    }

    # parse inputs
    my $_resource_path = '/network_sizes/search';
    $_resource_path =~ s/{format}/json/; # default format to json
