package VericredClient::ApiFactory;
...
usesub VericredClient::Object;

use VericredClient::ApiClient;

=head1 Name

	VericredClient::ApiFactory - constructs APIs to retrieve VericredClient objects
...
=head1 Synopsis
	...
	my $api_factory = VericredClient::ApiFactory->new( ... ); # any args for ApiClient constructor
							  
	# later...
	my $pet_api = $api_factory->get_api('Pet');  
	
	# $pet_api isa VericredClient::PetApi
...
my %_apis = map { $_ =~ /^VericredClient::(.*)$/; $1 => $_ } 
			grep {$_ =~ /Api$/} 
			usesub 'VericredClient';
...
sub new {
    my ($class, %p) = (shift, @_);
	$p{api_client} = VericredClient::ApiClient->instance(%p);			
