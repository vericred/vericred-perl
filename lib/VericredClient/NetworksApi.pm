...
# @param int $id Primary key of the network (required)
{
    ...
    'id' => {
        data_type => 'int',
        description => 'Primary key of the network',
        required => '1',
    },
    };
    __PACKAGE__->method_documentation->{ 'show_network' } = { 
    	summary => 'Network Details',
        params => $params,
        returns => 'NetworkDetailsResponse',
        };
}
...
sub show_network {
    ...
    unless (exists $args{'id'}) {
      croak("Missing the required parameter 'id' when calling show_network");
    }

    # parse inputs
    my $_resource_path = '/networks/{id}';
    $_resource_path =~ s/{format}/json/; # default format to json
    ...
    my $_response_object = $self->{api_client}->deserialize('NetworkDetailsResponse', $response);
    return $_response_object;
}
