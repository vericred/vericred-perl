...
# @param RequestProvidersSearch $body  (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_providers_0' } = { 
    	summary => 'Find Providers',
        params => $params,
        returns => 'ProvidersGeocodeResponse',
        };
}
...
sub get_providers_0 {
    ...
    # verify the required parameter 'body' is set
    unless (exists $args{'body'}) {
      croak("Missing the required parameter 'body' when calling get_providers_0");
    }

    # parse inputs
    my $_resource_path = '/providers/search/geocode';
    $_resource_path =~ s/{format}/json/; # default format to json
    ...
    my $_response_object = $self->{api_client}->deserialize('ProvidersGeocodeResponse', $response);
    return $_response_object;
}
